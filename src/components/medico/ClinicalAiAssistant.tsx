...
import { Brain, Sparkles, AlertTriangle, ShieldCheck, CheckCircle2, FileText, PlusCircle, Activity, ChevronDown, ChevronUp, RefreshCw } from 'lucide-react';
import { Patient, Anamnesis, AntropometricParams, ClinicalVitals, VitalsEvaluation, LabExam, DiagnosticHypothesis, ClinicalAlert } from '../../types/ppueri';
...
export const ClinicalAiAssistant: React.FC<ClinicalAiAssistantProps> = ({
  ...
  const [analysis, setAnalysis] = useState(() =>
    runPediatricCdssAnalysis(patient, anamnesis, antropometry, vitals, vitalsEvaluations, exams)
  );

  const handleRefreshAnalysis = () => {
    setIsAnalyzing(true);
    setTimeout(() => {
      const updated = runPediatricCdssAnalysis(patient, anamnesis, antropometry, vitals, vitalsEvaluations, exams);
      setAnalysis(updated);
    ...
    <div className="bg-slate-900/90 backdrop-blur-xl border border-sky-500/30 rounded-2xl shadow-xl text-white overflow-hidden transition-all">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-sky-200/80 mt-0.5">
              Análise cruzada de anamnese, Z-scores, sinais vitais e marcadores laboratoriais
            </p>
          </div>
        </div>
      ...
      {isExpanded && (
        <div className="p-5 space-y-5 bg-slate-950/60 text-slate-200 text-xs">
                ...
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-[10px] px-2 py-0.5 rounded-md border ${getLikelihoodBadge(item.likelihood)}`}>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {analysis.clinicalAlerts.map((alert, idx) => {
                  ...
                  return (
                    <div key={idx} className={`p-3 rounded-xl border flex items-start gap-3 ${style.bg}`}>
                      {style.icon}
                      <div className="space-y-0.5">
                        <div className={`font-bold text-xs ${style.titleColor}`}>{alert.title}</div>
          ...
          {/* Recommended Exams if any */}
          {analysis.recommendedExams.length > 0 && (
            <div className="p-3.5 bg-slate-900 border border-slate-800 rounded-xl space-y-2">
              <span className="font-bold text-sky-300 text-xs block">Exames Complementares Sugeridos para Investigação:</span>
              <div className="flex flex-wrap gap-2">
                {analysis.recommendedExams.map((ex, i) => (
                  <span key={i} className="bg-sky-950/80 text-sky-200 border border-sky-800/80 text-[11px] font-medium px-2.5 py-1 rounded-lg">
                    {ex}
                  </span>
                ))}
              </div>
          ...
          {analysis.carePlanSuggestions && (
            <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                ...
                {onApplyPrescriptionSuggestion && analysis.carePlanSuggestions.prescriptionsText && (
                  <button
                    onClick={() => onApplyPrescriptionSuggestion(analysis.carePlanSuggestions.prescriptionsText || '')}
              ...
              {analysis.carePlanSuggestions.prescriptionsText && (
                <div>
                  <span className="text-[11px] text-slate-400 font-semibold block">Prescrição Terapêutica Sugerida:</span>
              ...
              {analysis.carePlanSuggestions.feedingAdvice && (
                <div>
                  <span className="text-[11px] text-slate-400 font-semibold block">Orientação Nutricional Recomendada:</span>
                  <p className="text-slate-200 text-xs mt-0.5">{analysis.carePlanSuggestions.feedingAdvice}</p>
                </div>
              )}
            </div>
          )}
