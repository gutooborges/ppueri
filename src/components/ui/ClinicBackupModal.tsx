...
import { Download, Upload, Database, AlertTriangle, CheckCircle, RefreshCw, X, FileJson, Shield } from 'lucide-react';
import { Patient, Consultation, Appointment, VaccineRecord, PediatricNotification } from '../../types/ppueri';
import { exportClinicBackup, importClinicBackup, resetToDemoData } from '../../lib/storage-sync';
    ...
    reader.onload = (event) => {
      try {
        ...
        if (restored) {
          ...
          setStatusMessage({
            text: `Backup restaurado com sucesso! ${restored.patients.length} pacientes carregados.`,
            type: 'success',
          });
        } else {
      ...
      <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-2xl border border-sky-200 space-y-5 animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="p-3 bg-sky-50/60 border border-sky-200 rounded-xl text-[11px] text-sky-900 space-y-1">
          <div className="font-extrabold flex items-center gap-1.5 text-sky-950">
            ...
            <span>Armazenamento Criptografado & Resiliência Offline</span>
          </div>
          <p className="text-sky-800 leading-relaxed">
            Seus dados são salvos continuamente no navegador com sincronização resiliente. Para ambientes multi-usuário com banco na nuvem (PostgreSQL / Supabase), configure as variáveis no arquivo <code>.env</code>.
          </p>
        </div>
        ...
        <div className="pt-2 border-t border-sky-100 flex items-center justify-between">
          <button
            ...
            className="flex items-center gap-1 text-[11px] text-slate-500 hover:text-red-600 font-semibold transition-colors"
          >
