...
export const ClinicBackupModal: React.FC<ClinicBackupModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white/85 backdrop-blur-sm border border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-150">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-slate-100 text-slate-700 rounded-xl">
              <Database strokeWidth={1.75} className="w-5 h-5 text-slate-600" />
            </div>
            <div>
        ...
        {statusMessage && (
          ...
          >
            {statusMessage.type === 'success' ? (
              <CheckCircle strokeWidth={1.75} className="w-4 h-4 text-blue-600 shrink-0" />
            ) : (
              <AlertTriangle strokeWidth={1.75} className="w-4 h-4 text-amber-700 shrink-0" />
            )}
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-slate-50/80 border border-slate-200/80 rounded-xl p-3 text-center">
            <span className="text-[11px] text-slate-700 font-bold block">Pacientes</span>
            <span className="text-lg font-extrabold text-slate-900">{patients.length}</span>
          </div>
          <div className="bg-slate-50/80 border border-slate-200/80 rounded-xl p-3 text-center">
            <span className="text-[11px] text-slate-700 font-bold block">Consultas</span>
            <span className="text-lg font-extrabold text-slate-900">{consultations.length}</span>
          </div>
          <div className="bg-slate-50/80 border border-slate-200/80 rounded-xl p-3 text-center">
            <span className="text-[11px] text-slate-700 font-bold block">Agendamentos</span>
            <span className="text-lg font-extrabold text-slate-900">{appointments.length}</span>
          </div>
        </div>
        ...
        <div className="space-y-2.5">
          ...
          >
            <div className="flex items-center gap-2.5">
              <Download strokeWidth={1.75} className="w-4 h-4 text-blue-400 group-hover:scale-110 transition-transform" />
              <div className="text-left">
                <div>Exportar Backup Completo (JSON)</div>
                <div className="text-[10px] text-blue-300 font-normal">Baixa arquivo com todos os prontuários, vacinas e anamneses</div>
              </div>
            </div>
            <FileJson strokeWidth={1.75} className="w-4 h-4 text-blue-400" />
          </button>
          ...
          <button
            ...
            className="w-full flex items-center justify-between p-3.5 bg-slate-50 hover:bg-slate-100/80 border border-slate-200 text-slate-900 rounded-xl transition-all font-bold text-xs group hover:-translate-y-0.5 hover:shadow-md"
          >
            <div className="flex items-center gap-2.5">
              <Upload strokeWidth={1.75} className="w-4 h-4 text-slate-600 group-hover:scale-110 transition-transform" />
              <div className="text-left">
                <div>Restaurar / Importar Backup</div>
                <div className="text-[10px] text-slate-600 font-normal">Carrega base de dados a partir de arquivo JSON exportado</div>
              </div>
            </div>
            <FileJson strokeWidth={1.75} className="w-4 h-4 text-slate-600" />
          </button>
        </div>

        {/* LGPD & Cloud Security Notice */}
        <div className="p-3 bg-slate-50/60 border border-slate-200 rounded-xl text-[11px] text-slate-900 space-y-1">
          <div className="font-extrabold flex items-center gap-1.5 text-slate-900">
            <Shield strokeWidth={1.75} className="w-3.5 h-3.5 text-slate-600" />
